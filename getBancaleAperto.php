    ...
    function getBancale($conn)
    {
        ...
        if($result2==TRUE)
        {
            while($row2=sqlsrv_fetch_array($result2))
            {
                ...
                $query="SELECT DISTINCT dw_produzione.dbo.ordini_di_produzione.nome, dw_produzione.dbo.distinta_ordini_di_produzione.numero_cabina, db_tecnico.dbo.pannelli.codice_pannello, dbo.pannelli_prodotti.id_distinta, dbo.pannelli_prodotti.id_pannello_prodotto, dw_produzione.dbo.filtro_pannelli.elettrificato
                        ...
                        WHERE (dbo.pannelli_prodotti.bancale = ".$row2['id_bancale'].") ORDER BY dbo.pannelli_prodotti.id_pannello_prodotto DESC";
                $result=sqlsrv_query($conn,$query);
                if($result==TRUE)
                {
