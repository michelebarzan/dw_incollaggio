<?php
    ...
    TOP (100) PERCENT dw_produzione.dbo.distinta_ordini_di_produzione.numero_cabina, ISNULL(pannelli_caricati.pannelli_caricati, 0) AS pannelli_caricati, ISNULL(totale_pannelli_ordini_di_produzione.totale_pannelli, 0) 
    AS totale_pannelli
FROM            dw_produzione.dbo.ordini_di_produzione INNER JOIN
    dw_produzione.dbo.distinta_ordini_di_produzione ON dw_produzione.dbo.ordini_di_produzione.id_ordine_di_produzione = dw_produzione.dbo.distinta_ordini_di_produzione.ordine_di_produzione LEFT OUTER JOIN
        (SELECT        COUNT(db_tecnico.dbo.pannelli.id_pannello) AS totale_pannelli, distinta_ordini_di_produzione_1.numero_cabina
          ...
          HAVING         (distinta_ordini_di_produzione_1.ordine_di_produzione = 305)) AS totale_pannelli_ordini_di_produzione ON 
